...
(function () {
  ...
  function applyLoginSession() {
    if (!sessionStorage.getItem("mc_logged_in")) return;
    var role = sessionStorage.getItem("mc_login_role");
    var user = sessionStorage.getItem("mc_login_username");
    var roleSelect = document.getElementById("roleSelect");
    if (role && roleSelect) {
      var opt = roleSelect.querySelector('option[value="' + role + '"]');
      if (opt) roleSelect.value = role;
    }
    var bar = document.getElementById("mcUserBar");
    var label = document.getElementById("mcUserLabel");
    if (bar && label) {
      bar.hidden = false;
      label.textContent = user ? "Signed in: " + user : "Signed in";
    }
    var logout = document.getElementById("mcLogout");
    if (logout) {
      logout.addEventListener("click", function () {
        sessionStorage.removeItem("mc_logged_in");
        sessionStorage.removeItem("mc_login_role");
        sessionStorage.removeItem("mc_login_username");
        sessionStorage.removeItem("mc_login_type");
      });
    }
  }
